 ...
 * getPlannerEvents() / getRankingEvents() / getElectionEvents() share a
 * single load cycle.
 */
...
export async function getElectionEvents(): Promise<EventPlugin[]> {
const all = await getAllUnifiedEvents();
return all
        .map(config => config.plugin)
        .filter(e => e.features?.election?.enabled);
}